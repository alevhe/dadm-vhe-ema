...
def _data_replace(df, column_name):
    replacement = {None: "NA"}
...
def _read_file(filename):
    ...
    df = _data_encode(df, 'MSZoning')
    _data_replace(df, 'FireplaceQu')
    df = _data_encode(df, 'FireplaceQu')
    df = _data_encode(df, 'Functional')
